    ...
    try:
        ...
        if inventory_data:
            ...
            if items:
                ...
                items_text = "\n".join(
                    [f"**{num}.** {item}" for num, item in items.items()]
                )
...
async def send_single_prompt(
    bot, interaction, tribute_id, guild_id, prompt_image_dir, storage=None
):
    """
...
async def send_all_prompts_concurrent(
    bot, interaction, guild_id, prompt_image_dir, storage=None
):
    """
